...
class PomodoroUI:
    def __init__(self, root: tk.Tk):
        ...
        self.root.geometry("450x350")  # Width x Height
    ...
    def _create_widgets(self):
        ...
        self.status_label = ttk.Label(
            self.main_frame, textvariable=self.status_string, font=("Helvetica", 14)
        )
        ...
        self.work_spinbox = ttk.Spinbox(
            self.settings_frame, from_=1, to=60, width=5, textvariable=self.work_var
        )
        self.work_spinbox.grid(row=0, column=1, padx=5)

        # Break Duration Input
        ttk.Label(self.settings_frame, text="Break (min):").grid(
            row=0, column=2, padx=5
        )
        self.break_var = tk.StringVar(value="5")
        self.break_spinbox = ttk.Spinbox(
            self.settings_frame, from_=1, to=30, width=5, textvariable=self.break_var
        )
        ...
        # 1. Create a brand new frame packed below the first one
        self.secondary_button_frame = ttk.Frame(self.main_frame)
        self.secondary_button_frame.pack(pady=5)

        # Add the new Stop Alarm (Snooze) button
        self.stop_alarm_btn = ttk.Button(self.secondary_button_frame, text="Stop Alarm 🔇")
        # Change this line!
        self.stop_alarm_btn.pack(side=tk.LEFT, padx=5)
        # Add the new Skip button
        self.skip_btn = ttk.Button(self.secondary_button_frame, text="Skip session")
        # Change this line!
    ...
    def update_display(self, formatted_time: str):
        ...
        """
        # Update the self.time_string with the new formatted_time.
